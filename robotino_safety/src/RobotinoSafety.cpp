...
RobotinoSafety::RobotinoSafety( )
    ...
    e1_viz_pub_ = nh_.advertise< visualization_msgs::Marker >(
        "inner_ellipse_marker", 10 );
    e2_viz_pub_ = nh_.advertise< visualization_msgs::Marker >(
        "outer_ellipse_marker", 10 );

    robotino_cmd_vel_sub_ = nh_.subscribe(
        "/robotino_cmd_vel", 1, &RobotinoSafety::robotinoCmdVelCallback, this );
    bumper_sub_ =
        nh_.subscribe( "/bumper", 1, &RobotinoSafety::bumperCallback, this );
    scan_sub_ =
        nh_.subscribe( "/scan", 1, &RobotinoSafety::scanCallback, this );
    ...
    tfListener_.lookupTransform(
        "/base_link", "/laser_link", ros::Time( 0 ), tfTMP );
    transLaserScan.setIdentity( );
    transLaserScan.setBasis( tfTMP.getBasis( ) );
    transLaserScan.setOrigin( tfTMP.getOrigin( ) );
    transLaserScan.setRotation( tfTMP.getRotation( ) );
...
void RobotinoSafety::robotinoCmdVelCallback(
    ...
    if( msg->linear.x == 0.0f && msg->linear.y == 0.0f
        && msg->angular.z == 0.0f ) {
    ...
    if(slow_laser_){
        cmd_vel_msg_.linear.x *= 0.5f;
        cmd_vel_msg_.linear.y *= 0.5f;
        cmd_vel_msg_.angular.z *= 0.5f;
    }
...
void RobotinoSafety::scanCallback( const sensor_msgs::LaserScanConstPtr& msg ) {
    ...
    try {
        tf::StampedTransform tfTMP( transLaserScan,
                                    ros::Time::now( ) + ros::Duration( 0.25 ),
                                    "/base_link",
                                    "/laser_link" );
        tfListener_.setTransform( tfTMP );
        projector_.transformLaserScanToPointCloud(
            "/base_link", *msg, cloud, tfListener_ );
...
void RobotinoSafety::inE2( geometry_msgs::Point32 point ) {
    double check = pow( ( point.x / e2_major_radius_ ), 2 )
                   + pow( ( point.y / e2_minor_radius_ ), 2 ) - 1;
...
double RobotinoSafety::solveE1( geometry_msgs::Point32 point ) {
    return ( pow( ( point.x / e1_major_radius_ ), 2 )
             + pow( ( point.y / e1_minor_radius_ ), 2 ) - 1 );
}
